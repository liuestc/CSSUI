...
var share = require('../../../libs/common/js/share.js');

var host = /h5.chelun.com/.test(window.location.host) ? '//chelun.eclicks.cn' : '//community.dev.chelun.com';
var tid = /h5.chelun.com/.test(window.location.host) ? '' : '';
var hostid = /h5.chelun.com/.test(window.location.host) ? '//promotion.chelun.com/index.php?c=GuangzhouCarShow&v=index&id=' : '//dev-promotion.chelun.com/index.php?c=GuangzhouCarShow&v=index&id=';
// const tid='';
var env = {
    env: '',
    is_login: false,
...
(function () {
    ...
    if (app_name) {
        ...
        chelunJSBridge.invoke('device', 'getLocation', {
            locationCallBackName: function locationCallBackName(res) {
                if (res.result == 1) {
...
function getMain(tid) {
    $.ajax({
        ...
        success: function success(res) {
            ...
            var content = topic.content.replace(/\n/ig, '<br>');
...
$("#writeMes").on("focus", function () {
    if (!env.ac_token) {
        chelunJSBridge.invoke('app', 'login', {
            loginCallBackName: function loginCallBackName(res) {
                if (res.result == 1) {
...
$(".push").on("click", function () {
    ...
    $.ajax({
        ...
        success: function success(res) {
            ...
            //todo 展示回复成功的动
            showSucc();
            var time = getDateDiff(post.mtime),
                html = '';
            var quote = '';

            if (post.quote_pid != 0) {
                var _quote = res.data.quote,
                    users = res.data.user;
                var quoteContent = _quote[post.quote_pid].content;
                var uid = _quote[post.quote_pid].uid;
                var _oid = _quote[post.quote_pid].oid;
                var quoteName = users[uid].nick;
                // let m=user.avatar;

                _quote = '<p class="quote">\n                        \u56DE\u590D<span>' + getFloor(_oid, true) + '</span><span>' + quoteName + '</span>: ' + quoteContent + '\n                    </p>';
...
function getPost(limit, tid, pos) {
    ...
    $.ajax({
        ...
        success: function success(res) {
            //todo 拉取分页数据填充列表
            env.postState = true;
            var posts = res.data.post || [],
                users = res.data.user || [],
                quotes = res.data.quote || [];
            res.data.pos ? env.pos = res.data.pos : env.postState = false;
            var html = '';
            for (var i = 0, len = posts.length; i < len; i++) {
                if (env.height && posts[i].oid <= env.height && env.height <= 8) continue;
                var uid = posts[i].uid;
                var time = getDateDiff(posts[i].mtime);
                var quote = '';
                if (posts[i].quote_pid != '0') {
                    var quoteContent = quotes[posts[i].quote_pid].content;
                    var _uid = quotes[posts[i].quote_pid].uid;
                    var oid = quotes[posts[i].quote_pid].oid;
                    var quoteName = users[_uid].nick;
                    quote = '<p class="quote">\n                        \u56DE\u590D<span>' + getFloor(oid, true) + '</span><span> ' + quoteName + '</span>: ' + quoteContent + '\n                    </p>';
                }
            ...
            $(".res").off('click').on("click", function () {
                if (env.env == 'web') {
                    ...
                    var _oid2 = $(this).data('floor');
                    env.oid = $(this).data('pid');
                    $("#reply_floor").empty().append("回复" + _oid2 + '楼');
                    showInput();
                }
            });
        },
