...
;(function (global, factory) {
	...
	var chelunJSBridge = {
		...
		invoke: function invoke(namespace, api, param) {
			namespace = String(namespace);
			api = String(api);
			var url = this.schema + '://' + namespace + '/' + api;
			if (param && (typeof param === "undefined" ? "undefined" : _typeof(param)) === 'object') {
				url += this._buildQuery(param);
			}

			this._sendRequest(url);
		},
		_buildQuery: function _buildQuery(param) {
			var seg = [],
			    frag = '',
			    uriSeg = '';
		...
		//生成回调号
		_createCallbackNo: function _createCallbackNo(fn) {
		...
		//发送客户端调用请求
		_sendRequest: function _sendRequest(uri) {
